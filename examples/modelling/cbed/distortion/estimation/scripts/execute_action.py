...
previous action, assuming the previous action had already been executed
successfully. Note that the ML model set being alluded to here is a trivial set
containing only one ML model. Upon successful completion of the script, a
dictionary representation of the ML model after training is saved to a file at
the file path
...
"first" set of ML model tests of the ML model trained in the action
...
sequence ``(small, medium, large)``, the aforementioned ML model will have been
tested against the ML testing dataset stored in the HDF5 file at the file path
``<top_level_data_dir>/ml_datasets/ml_datasets_for_ml_model_test_set_1/ml_datasets_with_cbed_patterns_of_MoS2_on_amorphous_C/ml_dataset_with_<disk_size>_sized_disks.h5``,
...
CBED patterns and context relevant to the discussion below. In constructing a
fake CBED pattern, one needs to specify a set of circular, undistorted CBED disk
supports, defined in :math:`\left(u_{x},u_{y}\right)`-space, sharing a common
disk radius, with their centers being specified in
...
CBED disks that appear in the fake CBED pattern are be obtained by distorting
the aforementioned set of undistorted disk supports according to the
aforementioned distortion field. Let :math:`\left\{
