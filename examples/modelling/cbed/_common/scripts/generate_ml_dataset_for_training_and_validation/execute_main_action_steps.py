...
``<ml_dataset_idx>`` is an integer that is used to label the individual ML
dataset to be generated, and ``<data_dir_1>`` is the absolute path to an
existing directory or one to be created, within which the output data is
located.
...
_parse_and_convert_cmd_line_args_err_msg_1 = \
    ("The correct form of the command is:\n"
     ...
     "--data_dir_1=<data_dir_1>\n"
     "\n"
     "where ``<ml_model_task>`` must be set to {}; ``<ml_dataset_idx>`` can be "
     "any nonnegative integer, and ``<data_dir_1>`` can be any valid absolute "
     "path to any valid existing directory or one to be created.")
...
kwargs = {"num_pixels_across_each_cbed_pattern": \
          ...
          "device_name": \
          None}
cbed_pattern_generator = module_alias.DefaultCBEDPatternGenerator(**kwargs)
