...
class DefaultDistortionModelGenerator(_cls_alias):
    ...
    via the method :method:`~fancytypes.Updatable.update`, a random numpy
...
class DefaultCBEDPatternGenerator(_cls_alias):
    ...
    :method:`~fancytypes.Updatable.update`, a random numpy generator ``rng`` is
...
class MLModel(_MLModel):
    ...
    :class:`emicroml.modelling.cbed.distortion.estimation.MLDataset.MLModelTrainer`. 

    After a ML model has been trained, users should use the method
    :meth:`emicroml.modelling.cbed.distortion.estimation.MLDataset.MLModel.make_predictions`
    ...
    architecture : `str`, optional
        ...
        :math:`\text{conv\_2d}\left(C_{1},C_{2},K,S\right)` is a 2D
        ...
        outputs; :math:`\text{mini\_batch\_norm\_2d}\left(C\right)` is a
        ...
        :math:`\text{conv\_2d}\left(C_{1},C_{2},1,1+N_{\downarrow}\right)` if
        ...
        .. figure:: ../_images/modelling/cbed/distortion/estimation/no_pool_resnet_middle_flow.png

           The no-pool residual network middle flow, where :math:`C_1`,
           :math:`K`, and :math:`\mathbf{N}_{\mathbf{B}}` are the number of
           input channels, the maximum kernel size, and the building block
           counts in the residual network stages of the middle flow
           respectively.
        ...
        Next, we introduce the no-pool residual network exit flow, defined
        graphically as:

        .. _modelling_cbed_distortion_estimation_no_pool_resnet_exit_flow:
        .. figure:: ../_images/modelling/cbed/distortion/estimation/no_pool_resnet_exit_flow.png

           The no-pool residual network middle exit, where :math:`F_1`,
           :math:`F_2`, and :math:`F_3` are the number of nodes in the third
           last, the second last, and the last layers respectively.
        ...
        :math:`\text{mini\_batch\_norm\_1d}\left(C\right)` is a mini-batch
        normalization layer of 1D inputs with :math:`C` input channels.

        Next, we introduce the generic no-pool residual network, defined
        graphically as:

        .. _modelling_cbed_distortion_estimation_no_pool_resnet:
        .. figure:: ../_images/modelling/cbed/distortion/estimation/no_pool_resnet.png

           The generic no-pool residual network, where :math:`C_1`, :math:`C_2`,
           :math:`K_1`, :math:`K_2`, :math:`\mathbf{N}_{\mathbf{B}}`, :math:`H`,
           :math:`W`, :math:`F_1`, and :math:`F_2` are the number of input
           channels of the entry flow, the number of output channels of the
           entry flow, the maximum kernel size in the entry flow, the maximum
           kernel size in the middle flow, the building block counts in the
           residual network stages of the middle flow, the height of the input
           tensor in pixels, the width of the input tensor in pixels, the number
...
class MLModelTrainer(_cls_alias):
    ...
    The current class represents machine learning (ML) model trainer that can be
    used to train via supervised learning ML models represented by the class
    :class:`emicroml.modelling.cbed.distortion.estimation.MLModel`.
...
class MLModelTester(_cls_alias):
    ...
    The current class represents machine learning (ML) model tester that can be
